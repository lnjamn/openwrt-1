...
restart(){
	...
	if [ $vt_enabled = 1 ]; then
		...
		uci add_list dhcp.@dnsmasq[0].server=127.0.0.1#7913
		uci delete dhcp.@dnsmasq[0].resolvfile
		uci set dhcp.@dnsmasq[0].noresolv=1
		uci commit dhcp
		cp /etc/pdnsd_gfw.cfg /var/etc/passwall/pdnsd/pdnsd.conf
		...
		uci del_list dhcp.@dnsmasq[0].server=127.0.0.1#7913
