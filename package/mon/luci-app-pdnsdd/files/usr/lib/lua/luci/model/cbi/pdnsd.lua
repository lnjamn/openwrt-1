...
if pdnsd_on and string.sub(listen_port,1,14) == "127.0.0.1#7913" then 
	state_msg=state_msg .. "，并恭喜你DNS转发正常"
elseif pdnsd_on then
	state_msg=state_msg .. "<b><font color=\"red\">但是当前DNS转发为｛" .. listen_port .."｝请到【DHCP/DNS】修改为【127.0.0.1#7913】，否则Pdnsd无效果</font></b>"
end
if resolv_file=="" then
...
m=Map("pdnsd",translate("Pdnsd"),translate("Pdnsd可以通过TCP协议进行DNS解析,可以方便的使用iptables进行透明代理，配合ipset、GFWList使用效果更佳。默认上游服务器为114DNS，可以在【DHCP/DNS】中设置【DNS转发】为【127.0.0.1#5053】即可将全局DNS请求交给pdnsd进行解析，由于pdnsd自带缓存，所以可以加快解析速度。另外你可以使用dig来检测DNS解析情况，方法dig @127.0.0.1 -p 5053 www.facebook.com ").. "<br><br>状态 - " .. state_msg)
